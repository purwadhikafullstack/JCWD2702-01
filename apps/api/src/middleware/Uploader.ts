        ...
        if (Array.isArray(uploadedFiles)) {
          ...
          uploadedFiles.forEach((item) => {
            if (item.size > 1000000) {
              throw {
                message: `${item.originalname} is too large, maximum file size is 1MB`,
              };
            }
          });
        }
  ...
  upload(req, res, function (err) {
    try {
      ...
      if (req.files) {
        ...
        const uploadedRoomtypeFiles = Array.isArray(req.files)
          ? req.files
          : req.files['roomtypeImages'];
