...
export const createListing = async (uid: string, listingData: any, roomTypeData: any, listingImages: any, roomtypeImages: any) => {
    return await prisma.$transaction(async (prisma) => {
        ...
        if (findTenant) {
            ...
            const createListingSlug = await prisma.listings.update({
                ...
                data: {
                    slug: `${createdListing.title} ${createdListing.id}`.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-')
                }
            })
            ...
            if (createdListing) {
                ...
                for (const item of roomTypeData) {
                    ...
                    if (item.room_facilities) {
                        const room_facilities = item.room_facilities.map((facilityId: number) => ({
