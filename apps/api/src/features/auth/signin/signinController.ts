...
export const signin = async (
  ...
  try {
    ...
    if (!findLoginUserByEmailResult.is_verified) {
    ...
    if (findLoginUserByEmailResult.password) {
      if (password) {
        const comparePasswordResult = await comparePassword({
          ...
          passwordFromDb: findLoginUserByEmailResult.password,
        });
        if (!comparePasswordResult) throw new Error('Password is incorrect!');
      } else {
        throw new Error('Password is required!');
      }
    } else if (password && findLoginUserByEmailResult.providersId != 1) {
      throw new Error(
...
export const persistSignin = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
