  ...
  try {
    ...
    if (findAccountByEmailResult.is_verified === false) {
      const accesstoken = await createToken({
        data: {
          uid: findAccountByEmailResult.uid,
          email: findAccountByEmailResult.email,
          isVerified: findAccountByEmailResult.is_verified,
        },
        expiresIn: '1h',
      });
      ...
      res.status(200).send({
        error: false,
        message:
          "Your account is not verified, We've sent you an email to verify your account!",
      });
    } else {
      const accesstoken = await createToken({
        data: {
      ...
      const resetPasswordHTML = fs.readFileSync(
        process.env.NODEMAILER_RESET_PASSWORD_TEMPLATE_PATH as string,
        'utf-8',
      );
      let ResetPasswordHTMLCompiled: any =
        await Handlebars.compile(resetPasswordHTML);
      ResetPasswordHTMLCompiled = ResetPasswordHTMLCompiled({
        username: email,
        link: `${process.env.WEB_URL}/reset-password/${accesstoken}`,
      });

      transporterNodemailer.sendMail({
        from: 'Roomer',
        to: email,
        subject: 'Reset your Roomer account password!',
